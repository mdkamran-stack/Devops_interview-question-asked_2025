...
Scan code and images for vulnerabilities >> Continuously rescan images in your registry (not just at build time).
...
Implement approval gates and audit logging >> Implement approval gates by requiring manual checks before production deploys, and enable audit logging (via CloudWatch or CI/CD logs) to track who approved.

Monitor pipeline activity with Cloud watch >> Monitor pipeline activity using centralized logging and monitoring tools like AWS CloudWatch, CI/CD audit logs to track builds, deployments, failures, and security events in real time.
